...
$ws_svr->configureWSDL ( NAMA_WS, 'urn:' . NAMA_WSDL );
...
if (strlen($vano)<$len_userid){  // ubah strlen nya menjadi panjang virtual account
	...
	$cek = mysql_query("SELECT * FROM mutasi_bmi WHERE refno='$refno'"); 
				...
				if (mysql_num_rows($sql)=='1') {
					...
					if (($bill/100) <> $tagihan){
						...
						if ($payment1==$tagihan AND $custname==$nama_siswa){
				 		 	mysql_query("INSERT INTO mutasi_bmi(refno,vacc,nama,kelas,paralel,rek,uang,ket,trxdate) 
										VALUES('$refno','$v_vano','$nama','$kelas','$paralel','BMI','$payment1','$ket','$trxdate')");
							mysql_query("UPDATE $tabel_tagihan SET bayar='Y', rek='BMI', refno='$refno', trxdate='$trxdate' WHERE (vacc='$v_vano' AND id='$id')");
							$return = '00';	
				...
				if (mysql_num_rows($sql)=='1') {
					...
					if ($custname==$nama_siswa){
					 	mysql_query("INSERT INTO mutasi_bmi(refno,vacc,nama,kelas,paralel,rek,uang,ket,trxdate) 
									VALUES('$refno','$v_vano','$nama','$kelas','$paralel','BMI','$payment1','$ket','$trxdate')");
						mysql_query("INSERT INTO $tabel_tagihan(id,tahun,vacc,nama,kelas,paralel,unit,tagihan,pembayaran,bulan,bayar,rek,refno,user,trxdate) 
									VALUES('1','$tahun','$v_vano','$nama','$kelas','$paralel','$unit','$payment1','$pembayaran','$bulan1','Y','BMI','$refno','BMI','$trxdate')");	
						$return = '00';	
					}else{
						$return = '15';
					}		
	...
	if (mysql_num_rows( $cek )<>1) {
		...
		switch ($jenis_tagihan){
			...
			if (mysql_num_rows($sql)=='1') {
				...
				if ($bill1=='000000'){
 	 				mysql_query("INSERT INTO mutasi_bmi(refno,vacc,nama,kelas,paralel,rek,uang,ket,trxdate) 
								VALUES('$refno','$v_vano','$nama','$kelas','$paralel','BMI','$pay1','$ket','$trxdate')");
...
$ws_svr->register ( 'inquiry', array ('refno' => 'xsd:string', 'vano' => 'xsd:string', 'trxdate' => 'xsd:string'), array ('return' => 'xsd:string' ), 'urn:' . NAMA_WSDL, 'urn:' . NAMA_WSDL . '#inquiry', 'rpc', 'encoded', 'Deskripsi fungsi inquiry' );
//Register Function payment to Service
$ws_svr->register ( 'payment', array ('refno' => 'xsd:string', 'vano' => 'xsd:string', 'trxdate' => 'xsd:string', 'custname' => 'xsd:string', 'bill' => 'xsd:string', 'payment' => 'xsd:string', 'ccy' => 'xsd:string'), array ('return' => 'xsd:string' ), 'urn:' . NAMA_WSDL, 'urn:' . NAMA_WSDL . '#payment', 'rpc', 'encoded', 'Deskripsi fungsi payment' );
//Register Function reversal to Service
$ws_svr->register ( 'reversal', array ('refno' => 'xsd:string', 'vano' => 'xsd:string', 'trxdate' => 'xsd:string', 'paymentdate' => 'xsd:string', 'bill' => 'xsd:string', 'payment' => 'xsd:string'), array ('return' => 'xsd:string' ), 'urn:' . NAMA_WSDL, 'urn:' . NAMA_WSDL . '#reversal', 'rpc', 'encoded', 'Deskripsi fungsi reversal' );
//Register Function echo1 to Service
$ws_svr->register ( 'echo1', array ('echodate' => 'xsd:string'), array ('return' => 'xsd:string' ), 'urn:' . NAMA_WSDL, 'urn:' . NAMA_WSDL . '#echo1', 'rpc', 'encoded', 'Deskripsi fungsi echo' );
//Register Function signon to Service
$ws_svr->register ( 'signon', array ('signondate' => 'xsd:string'), array ('return' => 'xsd:string' ), 'urn:' . NAMA_WSDL, 'urn:' . NAMA_WSDL . '#signon', 'rpc', 'encoded', 'Deskripsi fungsi signon' );
//Register Function signoff to Service
$ws_svr->register ( 'signoff', array ('signoffdate' => 'xsd:string'), array ('return' => 'xsd:string' ), 'urn:' . NAMA_WSDL, 'urn:' . NAMA_WSDL . '#signoff', 'rpc', 'encoded', 'Deskripsi fungsi signoff' );
